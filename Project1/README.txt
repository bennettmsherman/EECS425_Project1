...
- I have a total of 6 separate .java files that make up this assignment.
	1.) ChatClient.java - This file contains all of the NON GUI code for the client. If you so desire, the
		client can be executed in command line mode by running "java ChatClient <hostname> <port number>".
	...
	2.) ChatClientGui.java - This file is what you will use as the client. It is a GUI which allows
		...
		the box is selected, or by pressing the "Send" button.
		***When you first open the client, you need to connect to the server by pressing the "Commands" menu,
		then "Connect To Server." In the popup, you can enter the IP and hostname, but I have autoconfigured them, so
	...
	3.) ChatParticipant.java - For each thread of the server that handles communcation with a client,
		...
		its name, listen mode state, and delimiter.
	...
	6.) ChatServer.java - This class contains the implementation for my chat server. It is obviously
		...
		using a ReentrantLock. Note that the GUI/Client doesn't store much state data; the delimiter,
...
2.) How to use
	...
	3.) With the client GUI open, connect to the server. This is done by clicking "Commands" then "Connect To Server"
		The eecslinab1 server and my assigned port are preloaded. Click OK to initiate the connection.
	...
	6.) From here, you can use any of the commands described below in the syntax section OR, the FAR EASIER approach
		...
		proper control message and enter it into the message field. Just click Send once the message is ready to go.
	...
	8.) The delimiter: The default delimiter is the enter key (the delimiter is stored as "" in this case);
		by this, I mean that new lines are indicated by multiple
		messages, which are explicitly sent. If you use the "Set My Delimiter" option (which needs to be set for each
		reconnect to the server), when you type a message with the delimiter in it, the resulting output shown on the
		receiving end will be multiple lines split at delimiter, and the delimiter will not be shown.
		Example (with delimiter = -1)
		You: Hello!-1What's up?
		(on the receiving side)
		Ben (line 1): Hello!
		Ben (line 2): What's up!
...
3.) Command syntax
	0.) General: All commands MUST start with the "C0NTR0L:". Any messages that start with "C0NTR0L:" will be interpreted
		...
		-The message syntax is "C0NTR0L:<Command>=<Input>", where the equals sign and <Input> are only required for
		 setter commands. There is no space between "C0NTR0L:" and the command. For setter commands, all test
		 following the equals is considered to be part of the input for that command.
		-Each command submission to the server
		 results in an acknowledgement/feedback of some sort.
	...
	3.) Set delimiter
		...
		- Special syntax: If <new delimiter> is blank, the Enter key will be used
		- Ex: "C0NTR0L:SET DELIMITER=-1"
		- As described above, your message will be split on the delimiter. The
		  delimiter represents a newline. In other words, if your message has a
		  delimiter within it, it means that the receiver will see one message,
		  but with multiple lines.
	4.) Get delimiter
		- Command: "GET DELIMITER"
		- EX: "C0NTR0L:GET DELIMITER"
		- The server will tell the client what it knows the client's delimiter to be.
	...
	6.) Get a list of clients currently connected to the server
...
4.) Assumptions/Other
	1.) I'm not entirely sure if my implementation of a delimiter is what the assignment was requesting,
	    and it wasn't entirely clear. Taken literally, the assignment states that a delimiter should
	    represent the end of a message, but this didn't seem to make a lot of sense to me. This would
	    imply that everything after the delimiter is not sent, which didn't seem like the desired
	    solution. In other words, if delimiter == 1, "Ben-1Sherman" would only send "Ben" and "Sherman"
	    would be trashed. This is my delimiter for having a delimiter represent a line separator for
	    a message. 
